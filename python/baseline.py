...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("Embeddings") as scope:
        L = tf.get_variable("Embeddings",(len(self.vocab), self.config.embed_size))
      ...
      for i in range(0, len(inputs)):
        squeezedInputs.append(tf.squeeze(inputs[i]))

      inputs = squeezedInputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("Projection") as scope:
      U = tf.get_variable("weights", [self.config.hidden_size, len(self.vocab)])
      b2 = tf.get_variable("bias", [1, len(self.vocab)], initializer=tf.constant_initializer(0.0))

    outputs = []

    for rnn_output in rnn_outputs:

      output = tf.add(tf.matmul(rnn_output, U), b2)
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("RNN") as scope:
      ...
      b1 = tf.get_variable("bias", [1, self.config.hidden_size], initializer=tf.constant_initializer(0.0))
    ...
    for i in range(0, len(inputs)):
      ...
      # Perform dropout on the inputs
      et = tf.nn.dropout(et,self.dropout_placeholder)

      if i == 0:
        hPrev = self.initial_state
      else:
        hPrev = rnn_outputs[i-1]

      # Use initial state as h^(t-1)
      output = tf.sigmoid(tf.add(tf.add(tf.matmul(hPrev, H), tf.matmul(et, I)), b1))

      # Perform dropout on output
      output = tf.nn.dropout(output,self.dropout_placeholder)

      rnn_outputs.append(output)

    self.final_state = rnn_outputs[len(rnn_outputs)-1]
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  ### MODIFYING OUTSIDE OF YOUR CODE HERE BLOCK FOR WARMUP

  for token in tokens:
    feed = {
      model.input_placeholder: np.reshape(token,(1,1)),
      model.labels_placeholder: np.ones((1,1)),
      model.initial_state: state,
      model.dropout_placeholder: np.array([[config.dropout]])
    }

    state, y_pred = session.run([model.final_state, model.predictions[-1]], feed_dict=feed)

  ### END MODIFYING OUTSIDE OF YOUR CODE HERE BLOCK FOR WARMUP
  ...
  for i in xrange(stop_length):
    ...
    # Create a feed dict
    feed = {
      model.input_placeholder: np.reshape(tokens[-1],(1,1)),
      model.labels_placeholder: np.ones((1,1)),
      model.initial_state: state,
      model.dropout_placeholder: np.array([[config.dropout]])
    }
